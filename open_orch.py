...
class sample:
    nttonb_dict = {"C": 0, "Cs": 1, "Db": 1, "D": 2, "Ds": 3, "Eb": 3, "E": 4, "F": 5, "Fs": 6, "Gb": 6, "G": 7,
                   "Gs": 8, "Ab": 8, "A": 9, "As": 10, "Bb": 10, "B": 11}
    nbtont_dict = {nb: nt for nt, nb in list(nttonb_dict.items())}
...
def fill_vel(sample_map):
    for sample_list in list(sample_map.items()):
...
def sort_lgth(filename):
    path = ""
    nosuffix = filename.split(".")
    elem = nosuffix[0].split("_")
    ...
    if grp in oodict:
        for instru in oodict[grp]:
            ...
            if not os.path.exists(dst_file):
                ...
                url = oodict["input url"] + instru["url"]
                r = requests.get(url)
                with open(dst_file, 'wb') as outfile:
                    outfile.write(r.content)


            #urllib.request.urlretrieve(oodict["input url"] + instru["url"], dst_file)
                ...
                with open(sfz_file_name, 'w') as sfz_file:
                    ...
                    for sample_list in list(sample_map.items()):
                        for smpl in sample_list[1]:
                            ...
                            if instru["sort"] == "lgth":
                                sfz_file.write(
                                    "sample=" + instru["name"] + "/" + lgth_path + "/" + smpl.filename + "\n")
                            else:
